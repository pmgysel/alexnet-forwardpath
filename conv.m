...
%bias is a 4d matrix: 1 x 1 x 1 x N.
%kernelSize and stride are integers.
%pad specifies the number of pixels to (implicitly) add to each side of the
%input.
...
function [ top ] = conv( bottom, weight, bias, kernelSize, stride, pad, group )
    [Win,Hin,N]=size(bottom);
    [~,~,~,M]=size(weight);
    bottomPadded=zeros(Win+2*pad,Hin+2*pad,N);
    bottomPadded(pad+1:end-pad,pad+1:end-pad,:)=bottom;
    Wout=(Win+2*pad-kernelSize)/stride+1;
    Hout=(Hin+2*pad-kernelSize)/stride+1;
    top=zeros(Wout,Hout,M);
    %Add convolution.
    %Code is vectorized over N: For one specific output feature map,
    %convolve all input feature maps and the corresponding kernels and the
    %same time.
    switch group
